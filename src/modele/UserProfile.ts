export class UserProfile {
    ...
    private _vehicleType: "car" | "electric" | "motorcycle" | "bike"; // type du véhicule

    constructor(name: string, surname: string, vehicleType: "car" | "electric" | "motorcycle" | "bike") {
        this._name = name;
    ...
    canPark(properties: any): boolean {
        ...
        if (spaces < 1) return false; // pas de place

        // Filtrer selon le type de véhicule
        switch (this._vehicleType) {
            case "car":
                // exclure vélo, moto, privé
                if (restriction.includes("bike") || restriction.includes("motorcycle") || restriction.includes("private")) return false;
                break;

            case "electric":
                // autoriser uniquement les parkings "electric" ou "paid-for"
                if (!restriction.includes("electric") && !restriction.includes("paid-for")) return false;
                break;

            case "motorcycle":
                if (!restriction.includes("motorcycle")) return false;
                break;

            case "bike":
                if (!restriction.includes("bike")) return false;
                break;
        }
