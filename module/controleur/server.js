...
serve(async (req) => {
    ...
    if (req.method === "POST") {
        ...
        if (url.pathname === "/register") {
            ...
            const exist = await client.execute("SELECT id FROM users WHERE username = ? OR email = ?", [username, email]);
            ...
            await client.execute("INSERT INTO users (username, email, password) VALUES (?, ?, ?)", [username, email, password] // Ici tu peux hasher le mdp pour plus de sécurité
            );
        ...
        if (url.pathname === "/login") {
            ...
            const result = await client.execute("SELECT id FROM users WHERE username = ? AND password = ?", [username, password]);
