            ...
            for (const leg of route.legs) {
                if (leg.steps) {
                    for (const step of leg.steps) {
                        ...
                        instructions.push(new NavigationInstruction(type, modifier, streetName, distance, location));
                    }
                }
            }
        }
        // Convert OSRM response to Itinerary object
        return new Itinerary(route.geometry.coordinates, route.distance, route.duration, instructions);
    }
}
